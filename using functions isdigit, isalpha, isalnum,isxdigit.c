#include <stdio.h>
#include <ctype.h>

int main()
{
    printf("%s\n   %s%s\n   %s%s\n\n","According to isdigit: ",  //returns a true if c is a digit and false otherwise
           isdigit('8')?"8 is a ": "8 is not a ","digit"  ,
           isdigit('#')?"# is a ": "# is not a ","digit");

    printf("%s\n  %s%s\n  %s%s\n  %s%s\n  %s%s\n","According to isalpha",//returns a true if c is a letter and false otherwise
            isalpha('A')?"A is a ": "A is not a ", "letter",
           isalpha('b')?"b is a ": "b is not a ", "letter",
           isalpha('&')?"& is a ": "& is not a ", "letter",
           isalpha('4')?"4 is a ": "4 is not a ", "letter");

    printf("\n%s\n %s%s\n %s%s\n %s%s\n\n","According to isalnum",//returns a true if c is a digit or a letter and false otherwise
           isalnum('A')?"A is a ":"A is not a ", "digit or letter",
           isalnum('8')?"8 is a ":"8 is not a ", "digit or letter",
           isalnum('#')?"# is a ":"# is not a ", "digit or letter");

    printf("\n%s\n %s%s\n %s%s\n %s%s\n  %s%s\n  %s%s\n\n","According to isxdigit",//returns a true if c is a hexadecimal digit character and false otherwise
           isxdigit('F')? "F is a ":" F is not a ", "hexadecimal digit",
           isxdigit('j')? "J is a ":" J is not a ", "hexadecimal digit",
           isxdigit('7')? "7 is a ":" 7 is not a ", "hexadecimal digit",
           isxdigit('$')? "$ is a ":" $ is not a ", "hexadecimal digit",
           isxdigit('f')? "f is a ":" f is not a ", "hexadecimal digit");
    return 0;

}
